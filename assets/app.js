...
const STORAGE_KEY="dakser-card-v1";
const PROFILE_KEY="dakser-card-profile-v1";
...
function renderProfile(){const mono=$("#monogram"),img=$("#profilePhoto");mono.textContent=initials(current.name);if(profileImage){img.src=profileImage;img.hidden=false;mono.hidden=true}else{img.removeAttribute("src");img.hidden=true;mono.hidden=false}}
function render(){
...
function vcard(){const lines=["BEGIN:VCARD","VERSION:3.0",`FN:${vesc(current.name)}`,`ORG:${vesc("Dakser Bilgisayar")}`,`TITLE:${vesc(current.title)}`];if(current.phone1)lines.push(`TEL;TYPE=CELL:${vesc(digits(current.phone1))}`);if(current.phone2)lines.push(`TEL;TYPE=WORK:${vesc(digits(current.phone2))}`);if(current.email)lines.push(`EMAIL;TYPE=WORK:${vesc(current.email)}`);if(current.address)lines.push(`ADR;TYPE=WORK:;;${vesc(current.address)};;;;`);if(current.website)lines.push(`URL:${vesc(current.website)}`);lines.push(`NOTE:${vesc(current.unit)}`,"END:VCARD");return lines.join("\r\n")}
function saveVcf(){const blob=new Blob([vcard()],{type:"text/vcard;charset=utf-8"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=(current.name||"Dakser-Bilgisayar-Kartvizit").replace(/[\\/:*?\"<>|]/g,"-")+".vcf";document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function downloadQr(){const box=$("#qrBox"),canvas=box.querySelector("canvas"),img=box.querySelector("img");let href="";if(canvas)href=canvas.toDataURL("image/png");else if(img?.src)href=img.src;if(!href)return toast("Önce QR kod oluşturun");const a=document.createElement("a");a.href=href;a.download="Dakser-Bilgisayar-QR.png";document.body.appendChild(a);a.click();a.remove()}
